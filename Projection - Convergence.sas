libname OF "H:\OptimalFertility\Convergence";
...
%let world_saf=0.664294573;
		...
		if year<2060 then active=pop*(lf_rate+(lf_rate_goal-lf_rate)*((year-2020)/(2060-2020)));
		if year>=2060 then active=pop*lf_rate_goal;
		inactive=pop-active;
...
run;
      ...
      proc export data=of.depratiocompiled
         outfile="H:\OptimalFertility\Convergence\depratiocompiled_conv.csv"
         dbms=csv
         replace;
      run;
...
data of.optimal_fert;
    ...
    /* Create an array for the 12 columns */
...
run;
      ...
      proc export data=of.optimal_fert
         outfile="H:\OptimalFertility\Convergence\optimal_fert_conv.csv"
         dbms=csv
         replace;
      run;


PROC IMPORT OUT=of.charac 
            DATAFILE='H:\OptimalFertility\Convergence\characteristics_conv.csv'
            DBMS=csv REPLACE;
        GETNAMES=YES;
        guessingrows=100000;
RUN;
